...
class ListBalancesRequest(BaseModel):
    ...
    def serialize_model(self, handler):
        optional_fields = set(
            ["currency", "from", "limit", "testmode", "idempotency-key"]
        )
        nullable_fields = set(["currency", "from", "limit"])
        serialized = handler(self)
        m = {}
...
class ListBalancesResponse(BaseModel):
    next: Union[
        Callable[[], Optional[ListBalancesResponse]],
        Callable[[], Awaitable[Optional[ListBalancesResponse]]],
    ]
...
try:
    ListBalancesResponseBody.model_rebuild()
except NameError:
    pass
