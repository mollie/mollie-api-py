...
class ListAllSubscriptionsRequest(BaseModel):
    ...
    def serialize_model(self, handler):
        optional_fields = set(
            ["from", "limit", "profileId", "testmode", "idempotency-key"]
        )
        nullable_fields = set(["from", "limit", "profileId"])
        serialized = handler(self)
        m = {}
...
class ListAllSubscriptionsEmbedded(BaseModel):
    ...
    @model_serializer(mode="wrap")
    def serialize_model(self, handler):
        optional_fields = set(["subscriptions"])
        serialized = handler(self)
        m = {}
...
class ListAllSubscriptionsResponse(BaseModel):
    next: Union[
        Callable[[], Optional[ListAllSubscriptionsResponse]],
        Callable[[], Awaitable[Optional[ListAllSubscriptionsResponse]]],
    ]
...
try:
    ListAllSubscriptionsResponseBody.model_rebuild()
except NameError:
    pass
